...
public class GenerateMonthlyInvoicesHandlerTests
{
    ...
    [Fact]
    public async Task Handle_ShouldSkipContractsThatStartAfterTheReferenceMonth()
    {
        var plan = PlanBuilder.Build();
        var contract = ContractBuilder.Build(planId: plan.Id, startDate: ReferenceMonth.AddMonths(1));
